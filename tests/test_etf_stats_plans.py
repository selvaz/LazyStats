...
class TestShadowLoadsNoLiveComponent:
    """Importing the runner and building a shadow plan must not pull the
    result depot or the Telegram connector into the process.

    Checked in a fresh subprocess against sys.modules: an in-process check
    would be meaningless once the test session has imported them for other
    reasons.
    """

    def test_building_a_shadow_plan_loads_neither_depot_nor_telegram(self, tmp_path):
...
from lazystats.etf_stats import load_config
m.build_shadow_plan(load_config(r"{EXAMPLE}"), r"{tmp_path}")
hits = [n for n in sys.modules if "io.depot" in n or "telegram" in n.lower()]
print("LOADED:" + ",".join(sorted(hits)))
""",
            encoding="utf-8",
        )
        env = dict(os.environ)
        ...
        assert loaded == [], f"shadow path pulled in live components: {loaded}"


class TestProductionDirectoryGuardIsCanonical:
    def test_a_different_case_is_still_the_reports_directory(self, runner):
        """Windows paths are case-insensitive; string equality is not."""
        assert runner._is_inside_reports_dir(runner.REPORTS_DIR.upper())
        assert runner._is_inside_reports_dir(runner.REPORTS_DIR.lower())

    def test_a_subdirectory_is_refused(self, runner):
        """reports/shadow/ is still production output."""
        assert runner._is_inside_reports_dir(os.path.join(runner.REPORTS_DIR, "shadow"))

    def test_an_unrelated_directory_is_allowed(self, runner, tmp_path):
        assert not runner._is_inside_reports_dir(str(tmp_path))

    def test_a_link_to_the_reports_tree_is_refused(self, runner, tmp_path):
        """A junction or symlink reaches the same tree under another name."""
        os.makedirs(runner.REPORTS_DIR, exist_ok=True)
        link = tmp_path / "alias"
        try:
            os.symlink(runner.REPORTS_DIR, link, target_is_directory=True)
        except (OSError, NotImplementedError) as exc:
            pytest.skip(f"this system refuses to create the link: {exc}")
        assert runner._is_inside_reports_dir(str(link))

    def test_cli_refuses_a_case_variant(self, runner):
        r = run_cli("--config", str(EXAMPLE), "--as-of", "2026-08-08",
                    "--dry-run", "--output-dir", runner.REPORTS_DIR.upper())
        assert r.returncode == 2
        assert "must not be the production reports directory" in r.stderr

    def test_cli_refuses_a_subdirectory(self, runner):
        r = run_cli("--config", str(EXAMPLE), "--as-of", "2026-08-08", "--dry-run",
                    "--output-dir", os.path.join(runner.REPORTS_DIR, "shadow"))
        assert r.returncode == 2
        assert "nor inside it" in r.stderr
